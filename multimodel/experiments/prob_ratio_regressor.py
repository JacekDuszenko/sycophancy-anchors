"""Probability ratio regressor - predicting prob_ratio from sentence-end activations.

This experiment predicts the model's belief (log(P(correct)/P(wrong))) at each
sentence position from the activations, following the original causal impact
regressor design.
"""
    ...
    def __init__(self, input_dim: int, hidden_dims: List[int] = None):
        ...
        for dim in hidden_dims:
            layers.extend([
                ...
                nn.LayerNorm(dim),  # Add LayerNorm for stability
                nn.ReLU(),
                nn.Dropout(0.1)
            ])
            prev_dim = dim
...
class ProbRatioRegressor:
    """Regressor for predicting prob_ratio from activations."""
    ...
    def _load_data(self) -> Tuple[torch.Tensor, torch.Tensor, List[int]]:
        """Load trajectory activations and prob_ratio targets."""
        act_dir = self.experiment_config.get_activations_dir(self.model_config.name)
        act_path = act_dir / "trajectory.safetensors"
        metadata_path = act_dir / "trajectory_metadata.json"

        if not act_path.exists():
            raise FileNotFoundError(
                f"Trajectory activations not found at {act_path}. "
                "Run activation extraction with --extract-trajectory first."
            )
        ...
        prob_ratios = tensors["prob_ratio"].to(DEVICE).float()
        target_layers = metadata["target_layers"]

        return activations, prob_ratios, target_layers

    def _gpu_standard_scaler(
        self, X_train: torch.Tensor, X_test: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        mean = X_train.mean(dim=0, keepdim=True)
        std = X_train.std(dim=0, keepdim=True) + 1e-8
        return (X_train - mean) / std, (X_test - mean) / std

    def _train_linear_lbfgs(
    ...
    def _train_mlp_adam(
        ...
        """Train MLP with Adam optimizer for stability (instead of LBFGS)."""
    ...
    def _run_cv_for_layer(
        ...
        for train_idx, val_idx in kf.split(X_cpu):
            ...
            # MLP with Adam (more stable than LBFGS)
            mlp_preds = self._train_mlp_adam(X_train_scaled, y_train, X_val_scaled)
    ...
    def run(self) -> List[ProbRatioResult]:
        np.random.seed(RANDOM_SEED)
        torch.manual_seed(RANDOM_SEED)
        if torch.cuda.is_available():
            torch.cuda.manual_seed_all(RANDOM_SEED)

        print(f"Loading trajectory data for {self.model_config.name}...")
        activations, prob_ratios, target_layers = self._load_data()

        print(f"Dataset statistics:")
        print(f"  Total sentences: {len(prob_ratios)}")
        print(f"  Activations shape: {activations.shape}")
        print(f"  Target (prob_ratio) range: [{prob_ratios.min():.3f}, {prob_ratios.max():.3f}]")
        print(f"  Target mean: {prob_ratios.mean():.3f}, std: {prob_ratios.std():.3f}")

        if len(prob_ratios) < 20:
        ...
        for layer_i in tqdm(range(n_layers), desc="Running prob_ratio regressor per layer"):
            layer_idx = target_layers[layer_i]

            # Shape: [N_sentences, hidden_size]
            X = activations[:, layer_i, :]
            y = prob_ratios
            ...
            results.append(ProbRatioResult(
        ...
        print("\n" + "=" * 70)
        print("PROB_RATIO REGRESSION SUMMARY BY LAYER")
        print("=" * 70)
        ...
        if mlp_results:
            best_mlp = max(mlp_results, key=lambda x: x.r2_score)
            print(f"\nBest layer: {best_mlp.layer} with R²={best_mlp.r2_score:.4f}")

        return results

    def save_results(self, results: List[ProbRatioResult]) -> Path:
        output_dir = self.experiment_config.get_results_dir(self.model_config.name)
        output_path = output_dir / "prob_ratio_results.csv"
